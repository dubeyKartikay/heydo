...
export default function Form() 
{
    ...
    const handleSubmit = (event) => 
    {
        ...
        const 
        {
          financer, rsa_length, motorNo, purchaseDate, model, salutation, fname,
          lname, email, addr1, addr2, addr3, tehsil, district, pincode, phoneNo,
          aadharNo, dob, nominee_name, relation_with_applicant, nominee_date_of_birth,
          city, state, state_code, district_code, nominee_age, gst_in
        } = event.target;
        ...
        // Send POST request with the data to the backend API
        axios.post('https://heydo-backend.vercel.app/create', data)
        .then(() => 
        {
        ...
        .catch(() => 
        {
            alert('An error occurred. Please reload the page and fill out the form again.');
        });

        // Show "Wait..." message while waiting for response
        alert('Wait...');
    }
    return (
        <div id="form-section">
        <form onSubmit={handleSubmit}>
            ...
            <div className="inp-block">
                <div className="inclusions">
                    ...
                    <input onClick={() => 
                    { 
                        setFormState(formState.map((ele, indx) => indx === 2 ? !ele : ele)) 
                    }} 
                    type="checkbox" 
                    name="rsa" 
                    id="rsa" 
                    ...
                    <input onClick={() => 
                    { 
                        setFormState(formState.map((ele, indx) => indx === 3 ? !ele : ele)) 
                    }} 
                    type="checkbox" 
                    name="insurance" 
                    id="insurance" 
            ...
            <div className="inp-block">
                ...
                <br />
                <select onChange = 
                {(event)=>setCurrState(event.target.value)}  
                type="select" 
                name="state" 
                id="state">
                {
            ...
            <div className="inp-block">
                <div className="inclusions">
                    <input onClick={() => 
                    { 
                        setFormState(formState.map((ele, indx) => indx === 0 ? !ele : ele)) 
                    }} 
                    type="checkbox" 
                    className='checkbox' 
                    name="finance" 
                    ...
                    <input onClick={() => 
                    { 
                        setFormState(formState.map((ele, indx) => indx === 1 ? !ele : ele)) 
                    }} 
                    type="checkbox" 
                    className='checkbox' 
                    name="gst" 
